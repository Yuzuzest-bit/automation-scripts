#!/usr/bin/env bash
# zk_extract_decision_flat_v2.sh
#
# TREE_VIEW.md から「🗳️付きの箇条書き行」だけを抽出して平坦化して残す。
# - VS16 (U+FE0F / UTF-8: EF B8 8F) を「行」と「検索キー」双方から除去して一致させる
# - 0件なら上書きしない
#
set -Eeuo pipefail
...
if [[ -z "$TARGET_FILE" || ! -f "$TARGET_FILE" ]]; then
  ...
  echo "      例: bash zk_extract_decision_flat_v2.sh dashboards/TREE_VIEW.md" >&2
  exit 1
fi
...
BEGIN{
  ...
  hit = 0
  h = 0
  mark = strip_vs16(MARK_IN)   # ★検索キー側も正規化する（ここが前回のバグ）
}
{
  line = strip_vs16($0)
...
rc="${rc:-0}"
if (( rc == 2 )); then
  echo "[ERR] '🗳️'（decision）を含む箇条書き行が 1件も見つかりません（上書きしません）。" >&2
  echo "      次で実物確認:" >&2
  echo "        grep -a -n \"🗳\" \"$TARGET_FILE\" | head" >&2
  exit 1
fi
(( rc == 0 )) || exit "$rc"
